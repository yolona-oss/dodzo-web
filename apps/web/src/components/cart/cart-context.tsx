...
import { ICartItem, IRestaurant } from '@dodzo-web/shared';
...
export function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    const addItem = async (productId: string, restaurantId: string, quantity: number) => {
        setLoading(true);
        try {
            if (user) {
                ...
                const product = await cartApi.getProductById(productId);
    ...
    const updateQuantity = async (productId: string, quantity: number) => {
        ...
        try {
            if (user) {
                const items = await cartApi.updateCartItemQuantity({productId, quantity});
