 ...
 */
export const isLocalHost = (host) => (host?.startsWith('localhost') ||
 ...
 */
export const getHost = (headers) => getHeaderValue(headers, 'x-forwarded-host')?.split(',')[0] || getHeaderValue(headers, 'host');
/**
 ...
 */
export const getProtocol = (headers) => {
    const host = getHost(headers);
    const protocol = (getHeaderValue(headers, 'x-forwarded-proto')?.split(',')[0] || getHeaderValue(headers, 'protocol'))?.toLowerCase();
 ...
 */
export const isHttps = (headers) => getProtocol(headers) === 'https';
/**
 ...
 */
export const getHostUrl = (headers) => {
    const host = getHost(headers);
